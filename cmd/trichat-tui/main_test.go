...
func TestResolveInteractiveExecuteBackend(t *testing.T) {
	if got := resolveInteractiveExecuteBackend("auto", []string{"npm test"}); got != "tmux" {
		t.Fatalf("expected auto backend to choose tmux when commands exist, got %q", got)
	}
	if got := resolveInteractiveExecuteBackend("auto", nil); got != "direct" {
		t.Fatalf("expected auto backend to choose direct without commands, got %q", got)
	}
	if got := resolveInteractiveExecuteBackend("tmux", nil); got != "tmux" {
		t.Fatalf("expected explicit tmux backend, got %q", got)
	}
}

func TestIsFanoutAutoDispatchRoute(t *testing.T) {
	if !isFanoutAutoDispatchRoute("/fanout-autoexec") {
		t.Fatalf("expected canonical fanout auto route to match")
	}
	if !isFanoutAutoDispatchRoute(" /FANOUT-AUTOEXEC ") {
		t.Fatalf("expected route matcher to be case/whitespace tolerant")
	}
	if isFanoutAutoDispatchRoute("/execute") {
		t.Fatalf("did not expect manual execute route to match auto-dispatch")
	}
}

func TestExtractExecuteCommandsFromObjectiveStructuredJSON(t *testing.T) {
	objective := `{
  "strategy":"execute quickly",
  "commands":["npm test","npm run build","npm test"]
}`
	commands := extractExecuteCommandsFromObjective(objective)
	if len(commands) != 2 {
		t.Fatalf("expected deduped command count=2, got %d (%v)", len(commands), commands)
	}
	if commands[0] != "npm test" {
		t.Fatalf("unexpected first command %q", commands[0])
	}
	if commands[1] != "npm run build" {
		t.Fatalf("unexpected second command %q", commands[1])
	}
}

func TestExtractExecuteCommandsFromObjectiveFallbackText(t *testing.T) {
	objective := `
- npm test
- run static checks
- go test ./...
...
`
	commands := extractExecuteCommandsFromObjective(objective)
	if len(commands) < 2 {
		t.Fatalf("expected at least 2 command-like entries, got %d (%v)", len(commands), commands)
	}
	if commands[0] != "npm test" {
		t.Fatalf("expected first parsed command npm test, got %q", commands[0])
	}
}

func TestDeriveAutoExecuteObjectivePrefersSelectedAgentCommands(t *testing.T) {
	responses := []agentResponse{
		{
			agentID: "cursor",
			content: `{"strategy":"safe route","commands":["npm run lint"]}`,
		},
		{
			agentID: "codex",
			content: `{"strategy":"ship route","commands":["npm test","npm run build"]}`,
		},
	}
	objective := deriveAutoExecuteObjective("codex", "fallback strategy", "decision summary", responses)
	if !strings.Contains(objective, `"commands":["npm test","npm run build"]`) {
		t.Fatalf("expected selected-agent response objective, got %q", objective)
	}
}

func TestDeriveAutoExecuteObjectiveFallsBackToStrategy(t *testing.T) {
	responses := []agentResponse{
		{agentID: "cursor", content: `{"strategy":"analysis-only","commands":[]}`},
	}
	objective := deriveAutoExecuteObjective("cursor", "fallback strategy text", "decision summary", responses)
	if objective != "fallback strategy text" {
		t.Fatalf("expected strategy fallback objective, got %q", objective)
	}
}

func TestDeriveOwnershipBoundExecutePlanFromStructuredTasks(t *testing.T) {
	objective := `{
  "strategy":"parallel ownership lanes",
  "tmux_tasks":[
    {"title":"tests lane","commands":["npm test"],"ownership_scope":"tests","ownership_mode":"read_only"},
    {"title":"build lane","commands":["npm run build"],"ownership_scope":"src","ownership_mode":"mutating"}
  ]
}`
	plan := deriveOwnershipBoundExecutePlan(objective, nil)
	if plan.source != "structured" {
		t.Fatalf("expected structured ownership plan source, got %q", plan.source)
	}
	if len(plan.tasks) != 2 {
		t.Fatalf("expected 2 ownership tasks, got %d", len(plan.tasks))
	}
	if plan.tasks[0].OwnershipScope == "" || plan.tasks[1].OwnershipScope == "" {
		t.Fatalf("expected ownership scopes on structured tasks")
	}
}

func TestBuildInteractiveTmuxTasksFromOwnedPlanIncludesGuardrails(t *testing.T) {
	plan := []ownershipBoundExecuteTask{
		{
			...
			OwnershipRule:  "verify tests only",
		},
	}
	tasks := buildInteractiveTmuxTasksFromOwnedPlan(plan, "codex", "thread-1", "turn-1")
	if len(tasks) != 1 {
		t.Fatalf("expected 1 tmux task from ownership plan, got %d", len(tasks))
	}
	metadata, ok := tasks[0]["metadata"].(map[string]any)
	if !ok {
		t.Fatalf("expected metadata map on tmux task")
	}
	if metadata["ownership_scope"] != "tests" {
		t.Fatalf("expected ownership_scope tests, got %#v", metadata["ownership_scope"])
	}
	if metadata["ownership_guardrail"] != true {
		t.Fatalf("expected ownership_guardrail=true, got %#v", metadata["ownership_guardrail"])
	}
}

func TestBuildAutoExecuteCycleObjectiveIncludesCycleTasks(t *testing.T) {
	base := `{"strategy":"ship","commands":["npm test","npm run build"]}`
	cycleObjective := buildAutoExecuteCycleObjective(base, "decision summary", 2, []string{"c1:ok"})
	if !strings.Contains(cycleObjective, `"cycle_kind":"review_fix_feature_verify"`) {
		t.Fatalf("expected iterative cycle marker in objective, got %q", cycleObjective)
	}
	if !strings.Contains(cycleObjective, `"tmux_tasks"`) {
		t.Fatalf("expected tmux_tasks block in objective, got %q", cycleObjective)
	}
}

func TestEvaluateExecuteReleaseGateFailsOnAutoSkip(t *testing.T) {
	passed, reasons := evaluateExecuteReleaseGate(executeReleaseGateInput{
		executionMode:      "auto_skipped",
		verifyStatus:       "skipped",
		autoDispatchOnly:   true,
		tmuxFallbackReason: "no executable commands",
	})
	if passed {
		t.Fatalf("expected auto skipped release gate to fail")
	}
	if len(reasons) == 0 {
		t.Fatalf("expected gate failure reasons")
	}
}
