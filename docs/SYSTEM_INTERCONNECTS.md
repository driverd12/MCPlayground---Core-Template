...
flowchart LR
  ...
  subgraph MCP["Local MCP Surface"]
    HTTP["HTTP transport<br/>/health /ready /office/api/* /federation/ingest / MCP POST"]
...
`desktop.context` is the shared MCP screen-context surface for Codex, Claude Code, Cursor, and other MCP clients. It can read a live Chronicle rolling buffer when that recorder is present and fresh, but Chronicle is optional and never becomes a hard runtime dependency; when Chronicle is unavailable or stale, `desktop.context` can fall back to a logged `desktop.observe` screenshot capture. The returned contract includes source, freshness, display/frame paths, noisy OCR hit summaries only when requested, authority summary, stale/unavailable reasons, recorder pid evidence, recommended next action, and the runtime event id that makes the observation visible across clients. `npm run doctor:chronicle` uses the same probe path, uses a 5-minute default freshness budget for sparse idle captures, and accepts current `codex_chronicle` pid files, current lock-file pid evidence, and legacy `codex_tape_recorder` pid files for smoother mixed-version Mac onboarding.
For N-host operation, every remote client is represented as its own `worker.fabric` host record on each peer that chooses to trust it. Hosts request access with `scripts/request_remote_access.mjs` or `POST /remote-access/request`; that path can only stage a pending host with sanitized metadata and never approves it. The script creates a per-host Ed25519 key under `~/.master-mold/identity`, sends the public key for local operator approval, and `scripts/mcp_tool_call.mjs` can sign each HTTP MCP request with `MASTER_MOLD_HOST_ID` plus `MASTER_MOLD_IDENTITY_KEY_PATH`. HTTP MCP sessions are bound to the validated network host identity instead of trusting caller-supplied `source_*` attribution. IP address is treated as a current locator, not a durable identity: approved-host matching can use the current address, the approved hostname resolving to that address, the approved MAC address observed through the LAN ARP table, and, when configured, the approved host public key. Signed remote-host requests also carry a timestamp and nonce; the server rejects stale proofs and nonce replays inside the freshness window. Approved remote hosts are scoped by `permission_profile` (`read_only`, `artifact_writer`, `task_worker`, or `operator`) before MCP tool calls reach the transport. Approved hosts can be used as `desktop.context host_id=<host>` context sources only when they also carry explicit desktop-context permission, and `scripts/remote_host_bootstrap_verify.sh <ssh-target> <repo-root>` provides the SSH/repo/build/context-probe handshake for each new host, including the first `Dans-MBP.local` host. There is no special two-host mode: repeat the same request, approval, bootstrap, and context probe loop for each whitelisted Mac or trusted node.

The federation stream is a peer-mesh sidecar, not a central hub. `scripts/federation_sidecar.mjs` captures a bounded local payload from that host's own MASTER-MOLD instance (`event.summary`, recent runtime events, and Chronicle/desktop-context metadata), signs every `POST /federation/ingest` request with the host identity, and publishes to the configured peer URLs in `MASTER_MOLD_FEDERATION_PEERS` or repeated `--peer` flags. Each receiving peer persists the payload as a `federation.ingest` runtime event and, when the host is already approved in `worker.fabric`, updates the host's federation freshness metadata. This creates an ad-hoc come-and-go tendril between approved MCP servers while keeping local SQLite/event logs canonical on every node.
Every ingested peer context/event carries a first-class identity envelope alongside the compact payload: requesting host ID, observed remote address, captured host ID/hostname/runtime/model, signature timestamp, receive timestamp, signature verification result, and approval/whitelist scope. The receiver derives that envelope from its approved network gate and verified host identity; it does not trust caller-provided `source_*` fields for session or event attribution.
