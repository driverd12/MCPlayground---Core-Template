...
The federation stream is a peer-mesh sidecar, not a central hub. `scripts/federation_sidecar.mjs` captures a bounded local payload from that host's own MASTER-MOLD instance (`kernel.summary`, recent runtime events, and Chronicle/desktop-context metadata), signs every `POST /federation/ingest` request with the host identity, and publishes to the configured peer URLs in `MASTER_MOLD_FEDERATION_PEERS` or repeated `--peer` flags. Each receiving peer persists the payload as a `federation.ingest` runtime event and, when the host is already approved in `worker.fabric`, updates the host's federation freshness metadata. This creates an ad-hoc come-and-go tendril between approved MCP servers while keeping local SQLite/event logs canonical on every node.
